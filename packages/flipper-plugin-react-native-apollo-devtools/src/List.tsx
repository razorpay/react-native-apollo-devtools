...
const sortData = (a: BlockType, b: BlockType) => a.name && b.name && a.name < b.name ? -1 : 1;

export function List({ data, activeTab, selectedItem, filter, onItemSelect, onTabChange }: {
    data: Data,
    activeTab: string,
    ...
    const filterData = (d: BlockType) => filter === '' || d.id?.includes(filter) || d.name?.includes(filter);
        ...
        <Layout.ScrollContainer>
            <Tabs defaultActiveKey="1" onChange={onTabChange}>
                ...
                <TabPane tab={TabsEnum.cache.value} key={TabsEnum.cache.key}>
                    {data?.cache?.filter(filterData).sort(sortData).map((d,i) => {
                        const active =
                ...
                <TabPane tab={TabsEnum.query.value} key={TabsEnum.query.key}>
                    {data?.queries?.filter(filterData).sort(sortData).map((d) => {
                        const active =
                ...
                <TabPane tab={TabsEnum.mutation.value} key={TabsEnum.mutation.key}>
                    {data?.mutations?.filter(filterData).sort(sortData).map((d) => {
                        const active =
